      ...
      <div class="w-full h-full text-gray-100 flex flex-col">
        <div class="flex justify-between items-center px-8 py-6">
          ...
          <div class="relative flex items-center space-x-4">
            ...
            />
            <font-awesome-icon :icon="['fas', 'search']" class="absolute left-1 top-1/2 transform -translate-y-1/2 text-gray-400" />
            <button 
              @click="exportAllClients" 
              class="flex items-center bg-green-600 text-white px-6 py-3 rounded-full shadow-lg hover:bg-green-500 transition-all duration-300 ease-in-out transform hover:scale-105"
            >
              ...
              <table class="w-full table-auto">
                ...
                <tbody>
                  <tr 
                    ...
                    :key="client.id" 
                    class="hover:bg-gray-700 transition-all duration-200 ease-in-out cursor-pointer"
        ...
        <div v-if="isPaymentTypeModalOpen" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-[1000]">
          <div v-if="paymentTypes && Array.isArray(paymentTypes)" class="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-auto">
            <SelectPaymentType 
          ...
          <div v-else class="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6 text-red-400">
            Error: Payment types are not available.
            <button @click="closePaymentTypeModal" class="bg-gray-600 text-white px-6 py-3 rounded-lg hover:bg-gray-500 mt-4">
              Close
            </button>
          </div>
        </div>
        ...
        <div v-if="isDeleteConfirmationOpen" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-[1010]">
          <div class="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div class="p-6">
              <h2 class="text-lg font-semibold text-gray-100 mb-4">Klientni o'chirish</h2>
              <p class="text-gray-300 mb-4">Haqiqatan ham <span class="font-bold">{{ clientToDelete?.first_name }} {{ clientToDelete?.lastname }}</span> ni o'chirmoqchimisiz?</p>

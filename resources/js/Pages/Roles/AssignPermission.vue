<template>
  <div class="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Ruxsat biriktirish</h2>
...
const submit = () => {
  router.post(route('roles.permissions.assign', props.role.id), form, {
    preserveState: true,
    preserveScroll: true,
    onSuccess: (page) => {
      form.permission_ids = [];
      errors.value = {};
      emit('assigned', page.props.role); // Emit the updated role
      emit('close');
    },
    onError: (err) => {
      errors.value = err;
